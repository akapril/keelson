...
- **文档 / 日历** —— 版本化文档（乐观并发、斜杠命令、KaTeX、文内 AI）；日历既是活动流水账（「记一笔」快录），也支持重复规则与**自然语言到点提醒**（含「提醒」才通知，关着也提醒）。
...
- **Web 远程访问** —— 设备配对 + Tailscale，从手机 / 浏览器访问本机工作台：会话 · 看板 · 日历 · 文档 · 多终端 · 会话记录 · 通知，**按能力逐项开关**（前端隐藏 + 后端强制）；token 认证、失败限流、默认关闭。
...
**agent 在跑长任务、你却得离开电脑？** 用手机 / 平板 / 另一台电脑瞄一眼进度、直接接着聊 `claude` / `codex`，不必守在机器前——这是 Keelson 的**陪伴式远程**，不是给公网裸暴露的远程管理。

- **能远程做什么**：多终端（跑并续 CLI）· 工作台会话 · 看板 · 日历（记一笔 / 到点提醒）· 文档 · **会话记录**（完整对话、可选择复制）· 通知 —— 移动优先响应式，底栏可拖拽排序。
- **你精确控制暴露什么**：每个能力（看板 / 日历 / 文档 / 终端…）在设置里**逐项开关**、敏感项默认关；**前端隐藏 + 后端强制**双重门控——关掉的能力，外部设备直连也拿不到。
- **双层安全**：① Tailscale 私有网（只有你同账号登录的设备可达，公网到不了）② 应用**配对 token**（外部设备输一次配对码，之后 token 鉴权、失败限流、可随时吊销）。**默认关闭**，需在设置里显式开启。
- **接入**：本机与远程设备装 Tailscale（同账号）→ 设置里开「Web 网关」并配对 → 经 `tailscale serve` 的 HTTPS 访问（`Secure` cookie 要求 HTTPS，见文档）。
- ⚠️ 远程终端可在本机执行任意命令 —— 只在**受信设备**上配对；设备丢失即在设置里吊销其 token（或直接关掉「终端」能力开关）。

**适合谁**：经常离开电脑、但想盯着 / 续接 agent 的人。**不适合**：把机器往公网上裸放——它不是为这个设计的。
